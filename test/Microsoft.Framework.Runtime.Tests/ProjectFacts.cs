...
namespace Microsoft.Framework.Runtime.Tests
{
    ...
    public class ProjectFacts
    {
        ...
        {
            var propertyInfo = typeof(Project).GetProperty(objectPropertyName);
            Assert.NotNull(propertyInfo);

            var expectValue = string.Format("this is a fake {0} at {1}", jsonPropertyName, DateTime.Now.ToShortTimeString());
